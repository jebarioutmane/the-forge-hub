/**
 * The Forge doodle — the one sanctioned ornament in the product (see the
 * forge-design standard, section 1). A hammer strikes the Forge mark into
 * existence, then THE FORGE letters drop in hot and cool to ink navy.
 * Plays once on mount, then rests. Honours prefers-reduced-motion by
 * rendering the finished state with no animation.
 */
const css = `
...
@media (prefers-reduced-motion: reduce){
  ...
  .fdBar,.fdWL,.fdWR{opacity:1;transform:none;animation:none}
  .fdLtr,.fdThe{opacity:1;fill:#0A2540;transform:none;animation:none}
  .fdHammer,.fdB1,.fdB2,.fdB3,.fdSpark{display:none;animation:none}
}
`;
...
export function ForgeDoodle() {
  return (
    ...
    >
      ...
      <text className="fdLtr" x="366" y="146" style={{ animationDelay: "3s" }}>R</text>
